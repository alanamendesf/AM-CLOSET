...
async function load() {
  try {
    ...
    if (!productsResponse.ok || !Array.isArray(productsData)) {
      ...
      document.getElementById('products').innerHTML = `<p>Erro ao carregar produtos: ${productsData.details || productsData.error || 'verifique a tabela products no Supabase.'}</p>`;
      products = [];
    } else {
